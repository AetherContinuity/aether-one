...
module pqc_ntt_full_banked_perf_tb;
  ...
  int global_cycle;
  int level_start_cycle;
  int prev_length;
  int level_cycles [128:2];  // length -> yhteensa sykleja tallä tasolla
  int bank_access_count [0:3];  // luku+kirjoitus yhteensa per pankki

  always @(posedge clk) begin
    global_cycle <= global_cycle + 1;
    if (dut.grant0) begin
      bank_access_count[dut.pb_a0] <= bank_access_count[dut.pb_a0] + 1;
      bank_access_count[dut.pb_b0] <= bank_access_count[dut.pb_b0] + 1;
    end
    if (dut.grant1) begin
      bank_access_count[dut.pb_a1] <= bank_access_count[dut.pb_a1] + 1;
      bank_access_count[dut.pb_b1] <= bank_access_count[dut.pb_b1] + 1;
    end
  end
  ...
  initial begin
    ...
    global_cycle = 0;
    level_start_cycle = 0;
    for (int b = 0; b < 4; b++) bank_access_count[b] = 0;
    clk = 0; reset = 1;
    ...
    @(posedge clk);
    start <= 1'b1;
    level_start_cycle = global_cycle;
    @(posedge clk);
    start <= 1'b0;
    run_wait(3000);
    level_cycles[128] = global_cycle - level_start_cycle;
    ...
    while (!$feof(fh) && scan_ok == 5) begin
      ...
      if (scan_ok == 5) begin
        ...
        start <= 1'b0;
        run_wait(3000);
        level_cycles[length] = level_cycles[length] + (global_cycle - level_start_cycle);
        if (!stage_done) begin
    ...
    if (error_count == 0) begin
      ...
      $display("--------------------------------------------------");
      $display("SUORITUSKYKYMITTAUS (M2 Vaihe 3d)");
      $display("--------------------------------------------------");
      $display("Sykliä per taso (teoreettinen ennuste: 192 jokaiselle):");
      foreach (level_cycles[l]) begin
        if (level_cycles[l] > 0) begin
          $display("  length=%0d: %0d sykliä (teoria: 192, ero: %0d)", l, level_cycles[l], level_cycles[l] - 192);
        end
      end
      $display("Kokonaissykliä (kaikki 7 tasoa yhteensä, ml. start-ylikustannus): %0d", global_cycle);
      $display("Teoreettinen ydinlaskenta (7 x 192): 1344 sykliä");
      $display("Ylikustannus (start-pulssit + reset): %0d sykliä (%0d käynnistystä)", global_cycle - 1344, 64);
      $display("");
      $display("Pankkien käyttöaste (luku+kirjoitus yhteensä):");
      for (int b = 0; b < 4; b++) begin
        $display("  pankki %0d: %0d käyttöä", b, bank_access_count[b]);
      end
    end else begin
