// pqc_ntt_inverse_12x_scaled_closure_tb.sv
//
...
module pqc_ntt_inverse_12x_scaled_closure_tb;
  ...
  logic [256*COEFF_W-1:0] result, result_scaled;
  int fh, scan_ok, error_count;

  pqc_ntt_final_scale #(.COEFF_W(COEFF_W)) scale_dut (.f_in(result), .f_out(result_scaled));
  ...
  initial begin
    ...
    for (int trial = 1; trial <= 12; trial++) begin
      ...
      #1;
      if (result_scaled !== inner_expect) begin
        $display("FAIL ajo %0d/12: SKAALATTU tulos poikkeaa golden-mallista (result_scaled[0:5]=%0d,%0d,%0d,%0d,%0d)",
                  trial, result_scaled[15:0], result_scaled[31:16], result_scaled[47:32], result_scaled[63:48], result_scaled[79:64]);
        error_count++;
      end else begin
        $display("OK ajo %0d/12: SKAALATTU tulos tasmaa golden-malliin", trial);
      end
    end

    $display("--------------------------------------------------");
    if (error_count == 0) begin
      $display("PASS: KAIKKI 12 ajoa antoivat oikean SKAALATUN tuloksen - VAHVISTAA etta aiempi poikkeama johtui yksinomaan puuttuvasta final_scale-vaiheesta vertailussa, ei aidosta NTT-bugista");
    end else begin
      $display("FAIL: %0d/12 ajoa epaonnistui - NTT-moduulin oma sisainen tila epailtava (odottamatonta - final_scale:n piti korjata tama)", error_count);
    end
    $display("--------------------------------------------------");
    $finish;
  end
