...
module pqc_shake256_tb;
  ...
  pqc_shake256 #(.MAX_BLOCKS(MAX_BLOCKS), .MAX_OUT_BYTES(MAX_OUT_BYTES)) dut (
  ...
  initial begin
    ...
    fh = $fopen("vectors/shake256_vectors.txt", "r");
    ...
    $display("--------------------------------------------------");
    if (error_count == 0) $display("PASS: SHAKE256 (%0d testitapausta, mukaan lukien ML-KEM PRF-tyylinen API-testi) tasmaa golden-malliin", case_count);
