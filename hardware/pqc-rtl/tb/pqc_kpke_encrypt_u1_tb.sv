// pqc_kpke_encrypt_u1_tb.sv
//
// M3 Issue #15 (jatko), K-PKE.Encrypt Vaihe 1: u[1] = NTT^-1(sum_j
// A[j][1]*y_hat[j]) + e1[0]. HUOM: A[j][1] - TRANSPONOITU indeksointi
...
module pqc_kpke_encrypt_u1_tb;
  ...
  logic [256*COEFF_W-1:0] e1_1;

  logic [256*COEFF_W-1:0] y_hat_expect [0:K-1];
  logic [256*COEFF_W-1:0] sum_before_e1_u1_expect;
  logic [256*COEFF_W-1:0] u1_expect;
  logic [256*COEFF_W-1:0] acc;
  logic [256*COEFF_W-1:0] ntt_inv_raw, ntt_inv_scaled;
  logic [256*COEFF_W-1:0] u1_got;
  ...
  initial begin
    ...
    // --- sum = sum_j A[j][1] * y_hat[j] (transponoitu indeksointi) ---
    acc = '0;
    for (int j = 0; j < K; j++) begin
      mntt_f = A_col1[j];
    ...
    if (acc !== sum_before_e1_u1_expect) begin
      $display("FAIL sum_before_e1_u1: poikkeaa golden-mallista (TRANSPONOINTITESTI: onko A[j][1] kaytetty A[0][j]:n sijaan?)");
      error_count++;
    end else $display("OK sum_before_e1 = sum_j A[j][1]*y_hat[j]: tasmaa golden-malliin (transponoitu indeksointi oikein)");
    ...
    if (u1_got !== u1_expect) begin
      $display("FAIL u[1]: poikkeaa golden-mallista");
      error_count++;
    end else $display("OK u[1] = NTT^-1(sum) + e1[0]: tasmaa golden-malliin (LOPULLINEN)");

    $display("--------------------------------------------------");
    if (error_count == 0) $display("PASS: u[1] (transponoitu matriisi-vektoritulo) tasmaa golden-malliin");
