// pqc_kpke_encrypt_u0_tb.sv
//
// M3 Issue #15 (jatko), K-PKE.Encrypt Vaihe 1: u[0] = NTT^-1(sum_j
...
module pqc_kpke_encrypt_u0_tb;
  ...
  logic [256*COEFF_W-1:0] e1_0;

  logic [256*COEFF_W-1:0] y_hat_expect [0:K-1];
  logic [256*COEFF_W-1:0] sum_before_e1_expect;
  logic [256*COEFF_W-1:0] u0_expect;
  logic [256*COEFF_W-1:0] acc;
  logic [256*COEFF_W-1:0] ntt_inv_raw, ntt_inv_scaled;
  logic [256*COEFF_W-1:0] u0_got;
  ...
  initial begin
    ...
    // --- sum = sum_j A[j][0] * y_hat[j] (transponoitu indeksointi) ---
    acc = '0;
    for (int j = 0; j < K; j++) begin
      mntt_f = A_col0[j];
    ...
    if (acc !== sum_before_e1_expect) begin
      $display("FAIL sum_before_e1: poikkeaa golden-mallista (TRANSPONOINTITESTI: onko A[j][0] kaytetty A[0][j]:n sijaan?)");
      error_count++;
    end else $display("OK sum_before_e1 = sum_j A[j][0]*y_hat[j]: tasmaa golden-malliin (transponoitu indeksointi oikein)");
    ...
    if (u0_got !== u0_expect) begin
      $display("FAIL u[0]: poikkeaa golden-mallista");
      error_count++;
    end else $display("OK u[0] = NTT^-1(sum) + e1[0]: tasmaa golden-malliin (LOPULLINEN)");

    $display("--------------------------------------------------");
    if (error_count == 0) $display("PASS: u[0] (transponoitu matriisi-vektoritulo) tasmaa golden-malliin");
