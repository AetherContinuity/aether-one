...
module pqc_mlkem_keygen_tb;
  ...
  initial begin
    ...
    // --- K-PKE.KeyGen(d) -> ekPKE, dkPKE (sama logiikka kuin
    // pqc_kpke_roundtrip_tb.sv:n oma KeyGen-vaihe) ---
    begin
      ...
      if (dk_got !== dk_expect) begin
        $display("FAIL dk: poikkeaa golden-mallista"); error_count++;
      end else $display("OK dk (=dkPKE||ek||H(ek)||z): tasmaa golden-malliin");
    end

    $display("--------------------------------------------------");
    if (error_count == 0) $display("PASS: ML-KEM.KeyGen_internal(d,z) -> (ek,dk) tasmaa golden-malliin");
