...
module pqc_kpke_keygen_full_tb;
  ...
  // --- ByteEncode12: t_hat/s_hat (16-bit/kerroin) -> tiivis 12-bit/kerroin -> tavut ---
  logic [3071:0] benc_in, benc_out;
  pqc_byteencode_dparam #(.D(12)) benc_dut (.f_in(benc_in), .b_out(benc_out));
  ...
  initial begin
    ...
    // --- Rivit 19-20: ekPKE = ByteEncode12(t_hat)||rho, dkPKE = ByteEncode12(s_hat) ---
    begin
      logic [255:0] rho_val;
      logic [8*(384*K+32)-1:0] ekPKE_got, ekPKE_expect;
      logic [8*(384*K)-1:0] dkPKE_got, dkPKE_expect;

      fh = $fopen("vectors/kpke_keygen_full_vectors.txt", "r");
      begin
        logic [255:0] d_unused;
        scan_ok = $fscanf(fh, "%h\n", d_unused);
      end
      for (int k = 0; k < 2*K; k++) begin
        logic [256*COEFF_W-1:0] tmp;
        scan_ok = $fscanf(fh, "%s %d\n", tag, idx_v);
        scan_ok = $fscanf(fh, "%h\n", tmp);
      end
      ...
      ekPKE_got = '0;
      for (int i = 0; i < K; i++) begin
        for (int c = 0; c < 256; c++) benc_in[c*12 +: 12] = t_hat[i][c*16 +: 16];
        #1;
        ekPKE_got[(i*384)*8 +: 384*8] = benc_out;
      end
      ekPKE_got[(2*384)*8 +: 32*8] = rho_val;

      dkPKE_got = '0;
      for (int i = 0; i < K; i++) begin
        for (int c = 0; c < 256; c++) benc_in[c*12 +: 12] = s_hat[i][c*16 +: 16];
        #1;
        dkPKE_got[(i*384)*8 +: 384*8] = benc_out;
      end

      if (ekPKE_got !== ekPKE_expect) begin
        $display("FAIL ekPKE: poikkeaa golden-mallista");
        error_count++;
      end else $display("OK ekPKE (ByteEncode12(t_hat)||rho, %0d tavua): tasmaa golden-malliin", 384*K+32);

      if (dkPKE_got !== dkPKE_expect) begin
        $display("FAIL dkPKE: poikkeaa golden-mallista");
        error_count++;
      end else $display("OK dkPKE (ByteEncode12(s_hat), %0d tavua): tasmaa golden-malliin", 384*K);
    end

    $display("--------------------------------------------------");
    if (error_count == 0) $display("PASS: KOKO K-PKE.KeyGen (d -> ekPKE+dkPKE) tasmaa golden-malliin");
    else begin $display("FAIL: %0d virhetta", error_count); $fatal(1); end
    $display("--------------------------------------------------");
    $finish;
  end
