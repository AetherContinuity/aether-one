...
// M4-FPGA-003A koe (kayttajan oma ehdotus): YKSI ARBITROITU
  ...
  always_ff @(posedge clk) begin
    ...
    br_b0 <= bank0[local_of({1'b0,read_addr})];
    br_b1 <= bank1[local_of({1'b0,read_addr})];
    br_b2 <= bank2[local_of({1'b0,read_addr})];
    br_b3 <= bank3[local_of({1'b0,read_addr})];
  ...
  always_comb begin
    case (read_bank_sel_reg)
      2'd0: read_data = br_b0;
      2'd1: read_data = br_b1;
      2'd2: read_data = br_b2;
      default: read_data = br_b3;
    endcase
  end

  // M4-FPGA-003A koe (kayttajan oma jatkoehdotus): LUKUPUOLEN
  // ARBITROINTI samalla periaatteella kuin kirjoituspuoli - koska
  // konfliktittomuustodistus (BANK_MAPPING_PROOF.md) takaa etta
  // KORKEINTAAN YKSI neljasta osoitteesta (a0,b0,a1,b1) osuu MIHIN
  // TAHANSA yksittaiseen pankkiin per sykli, JOKAINEN pankki
  // tarvitsee VAIN YHDEN lukuportin FSM-puolelta (ei neljaa).
  ...
  logic [5:0] fsm_raddr0, fsm_raddr1, fsm_raddr2, fsm_raddr3;
  logic dummy0, dummy1, dummy2, dummy3;
  always_comb begin
    dummy0 = arbitrate_read_addr(fsm_raddr0, 2'd0);
    dummy1 = arbitrate_read_addr(fsm_raddr1, 2'd1);
    dummy2 = arbitrate_read_addr(fsm_raddr2, 2'd2);
    dummy3 = arbitrate_read_addr(fsm_raddr3, 2'd3);
  end

  logic [15:0] fsm_b0, fsm_b1, fsm_b2, fsm_b3;
  always_ff @(posedge clk) begin
    fsm_b0 <= bank0[fsm_raddr0];
    fsm_b1 <= bank1[fsm_raddr1];
    fsm_b2 <= bank2[fsm_raddr2];
    fsm_b3 <= bank3[fsm_raddr3];
  end
