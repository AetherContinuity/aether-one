...
module pqc_ntt_stage_banked_1stage #(
  ...
  lane_fsm_1stage #(.COEFF_W(COEFF_W), .SPAD_AW(SPAD_AW), .READ_LATENCY(NTT_READ_LATENCY)) lane0 (
  ...
  lane_fsm_1stage #(.COEFF_W(COEFF_W), .SPAD_AW(SPAD_AW), .READ_LATENCY(NTT_READ_LATENCY)) lane1 (
    .clk(clk), .reset(reset), .start(start),
