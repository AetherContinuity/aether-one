...
module full_keygen_check_tb;
  ...
  initial begin
    ...
    begin
      ...
      $display("done=%0b, wait_cycles=%0d, state=%0d", done, wait_cycles, debug_state);
    end

    if (ek_out === ek_expect) $display("PASS: ek TASMAA taydellisesti golden-referenssiin!");
    else begin
      int diffs;
      diffs = 0;
      for (int i = 0; i < 800; i++) if (ek_out[i*8+:8] !== ek_expect[i*8+:8]) diffs++;
      $display("FAIL: ek EI tasmaa - %0d/800 tavua eroaa", diffs);
    end

    if (dk_out === dk_expect) $display("PASS: dk TASMAA taydellisesti golden-referenssiin!");
    else begin
      int diffs;
      diffs = 0;
      for (int i = 0; i < 1632; i++) if (dk_out[i*8+:8] !== dk_expect[i*8+:8]) diffs++;
