...
module check_postschedule_tb;
  ...
  initial begin
    ...
    @(posedge clk);
    start <= 1'b0;
    // Odota etta S_NTT_FWD_READ (tila 15) alkaa - aikataulu VALMIS, luku EI VIELA alkanut
    begin
      int wait_cycles;
      wait_cycles = 0;
      while (debug_state !== 5'd15 && wait_cycles < 15000) begin
        @(posedge clk);
        wait_cycles++;
      end
      $display("Saavutti S_NTT_FWD_READ (15) %0d syklin jalkeen", wait_cycles);
    end
    ...
    begin
      ...
      for (int i = 0; i < 256; i++) begin
        if (read_bank(i) !== s_hat0_expect[i*16+:16]) begin
          if (diffs < 10) $display("  [%0d] pankki=%0d, golden=%0d", i, read_bank(i), s_hat0_expect[i*16+:16]);
          diffs++;
        end
      end
      if (diffs == 0) $display("PASS: pankkien sisalto TASMAA s_hat[0]:hon HETI aikataulun jalkeen (ennen lukuvaihetta)");
      else $display("FAIL: %0d/256 arvoa vaarin JO PANKEISSA (aikataulun suoritus itsessaan viallinen taalla)", diffs);
    end
    $finish;
  end
endmodule
