...
module pqc_dilithium_verify_top2_tb;
  ...
  initial begin
    ...
    fh = $fopen("dilithium-rtl/verify_top2_test_vector_negative.txt", "r");
    scan_ok = $fscanf(fh, "%h\n", pk_val);
    scan_ok = $fscanf(fh, "%h\n", sig_val);
    scan_ok = $fscanf(fh, "%h\n", m_val);
    $fclose(fh);
    ...
    if (!verify_ok) begin
      $display("--------------------------------------------------");
      $display("PASS: KOKO ML-DSA-65.Verify_internal OIKEIN HYLKASI TURMELLUN ALLEKIRJOITUKSEN");
      $display("--------------------------------------------------");
    end else begin
      $display("--------------------------------------------------");
      $display("FAIL: Verify VIRHEELLISESTI hyvaksyi turmellun allekirjoituksen");
      $fatal(1);
    end
