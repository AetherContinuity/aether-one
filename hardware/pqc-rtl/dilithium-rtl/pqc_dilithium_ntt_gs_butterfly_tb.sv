// M5-DILITHIUM-001 DK1-testi: yksittaisen GS-butterflyn todennus.

`timescale 1ns/1ps

module pqc_dilithium_ntt_gs_butterfly_tb;

  localparam int CW = 23;
  logic [CW-1:0] a_in, b_in, zeta_in;
  logic [CW-1:0] a_out, b_out;

  pqc_dilithium_ntt_gs_butterfly dut (
  ...
  initial begin
    ...
    fh = $fopen("dilithium-rtl/gs_butterfly_test_vectors.txt", "r");
    ...
    $display("--------------------------------------------------");
    if (error_count == 0) $display("PASS: GS-butterfly tasmaa taydellisesti kaikille %0d testitapaukselle", case_count);
