...
module pqc_dilithium_verify_top2_tb;
  ...
  initial begin
    ...
    fh = $fopen("dilithium-rtl/verify_top2_test_vector_negative_pk.txt", "r");
    scan_ok = $fscanf(fh, "%h\n", pk_val);
    scan_ok = $fscanf(fh, "%h\n", sig_val);
    scan_ok = $fscanf(fh, "%h\n", m_val);
    $fclose(fh);
    ...
    begin
      ...
      else $display("Valmis %0d syklin jalkeen, verify_ok=%0b (odotettu: 0, turmeltu pk)", wait_cycles, verify_ok);
    end

    if (!verify_ok) begin
      $display("--------------------------------------------------");
      $display("PASS: KOKO ML-DSA-65.Verify_internal OIKEIN HYLKASI TURMELLUN JULKISEN AVAIMEN");
      $display("--------------------------------------------------");
    end else begin
      $display("--------------------------------------------------");
      $display("FAIL: Verify VIRHEELLISESTI hyvaksyi turmellun julkisen avaimen");
      $fatal(1);
    end
