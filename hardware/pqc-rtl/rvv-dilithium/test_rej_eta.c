...
int main(void) {
    FILE *fbuf = fopen("eta_input_buf.txt", "r");
    FILE *fgold = fopen("eta_golden.txt", "r");
    if (!fbuf || !fgold) { printf("FAIL: tiedostoja ei loydy\n"); return 1; }

    unsigned int buflen = 272;
    uint8_t *buf = malloc(buflen);
    for (unsigned int i = 0; i < buflen; i++) { int v; fscanf(fbuf, "%d", &v); buf[i] = (uint8_t)v; }
    fclose(fbuf);
    ...
    int32_t out[256] = {0};
    unsigned int ctr = rej_eta_rvv(out, 256, buf, buflen);
    ...
    for (unsigned int i = 0; i < check_n; i++) {
        if (out[i] != expected[i]) {
            errors++;
            if (errors <= 10) printf("[FAIL] i=%u got=%d expected=%d\n", i, out[i], expected[i]);
        }
    }
