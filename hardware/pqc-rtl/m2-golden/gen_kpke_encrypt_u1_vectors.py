#!/usr/bin/env python3
"""M3 Issue #15 (jatko), K-PKE.Encrypt Vaihe 1: u[1] = NTT^-1(sum_j A[j][1]*y_hat[j]) + e1[0].
...
# u[1] = NTT^-1( sum_j A[j][1] * y_hat[j] ) + e1[1]  (A^T - transponoitu)
acc = [0] * 256
for j in range(K):
    acc = mod_add_poly(acc, multiply_ntts(A[j][1], y_hat[j]))
sum_before_e1_u1 = list(acc)
u1 = mod_add_poly(ntt_inv(acc), e1_vec[1])

with open(os.path.join(outdir, "kpke_encrypt_u1_vectors.txt"), "w") as f:
    for j in range(K):
        f.write(f"A_transposed_col1_{j}\n{pack_coeffs(A[j][1]):x}\n")
    for j in range(K):
        f.write(f"y_hat_{j}\n{pack_coeffs(y_hat[j]):x}\n")
    f.write(f"sum_before_e1_u1\n{pack_coeffs(sum_before_e1_u1):x}\n")
    f.write(f"e1_1\n{pack_coeffs(e1_vec[1]):x}\n")
    f.write(f"u1\n{pack_coeffs(u1):x}\n")

print(f"u[1] laskettu: u1[0:5]={u1[:5]}")
